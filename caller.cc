...
__declspec(naked) uint64 fakecall(const caller* nowthis, void* argv)
  ...
  __asm
    ...
    mov     esi, esp                      // esi 指向寄存器保护。
    mov     ebx, dword ptr [esp + 4 * 13] // ebx 为 this 对象指针。 ->ad fd # esp # ret
    movzx   edx, [ebx].caller_argc        // edx 为参数个数。
    add     edx, 10                       //                ->ad fd func
    shl     edx, 2
    sub     esp, edx                      // 扩展局部堆栈。
    mov     edi, esp                      // edi 保存局部堆栈指针。

    xor     ecx, ecx
    mov     cl, 9                         //                ->ad fd
    rep movsd                             // 复制原始寄存器保护。
    mov     esi, dword ptr [esi + 4 * 5]  // esi 指向参数。 -># esp # ret this
    
    //////////////////////// 开始修改可能存在的对象指针寄存器。
    mov     ebp, [ebx].caller_this        // ebp 为对象指针，注意后继可能用于成员函数的调用。
    test    ebp, ebp                      // 对象指针存在才修改。
    jz      fakecall_copyregs
    movzx   edx, [ebx].caller_thisreg     // edx 为对象指针寄存器指示。
    mov     cl, 8                         // 八个寄存器，开始遍历。
fakecall_copythisregs:
    ...
    mov     dword ptr [esp + ecx * 4], ebp // copy 对象指针。对象指针只有一个，所以只修改一次。

    //////////////////////// 开始修改可能存在的寄存器传参。注意之前 esi 已经指向参数。
fakecall_copyregs:
    movzx   edx, [ebx].caller_regs        // edx 为寄存器传参指示。
...
fakecall_regs_ok:
    ...
    mov     edx, dword ptr [edx + ebp]    // 读取虚函数表，注意不用 mov edx, [ebp] 是为了不产生 00 编码，注意 [ebp + edx] 也不行。
    mov     eax, dword ptr [eax + edx]    // 读取虚成员函数。
fakecall_func:
    stosd                                 // 压入函数地址。
    
    //////////////////////// 开始复制剩余参数（除去寄存器参数）。注意在前面 esi 已经跳过可能的寄存器参数了。而 edi 已经很早就准备好了。
    movzx   ecx, [ebx].caller_argc        // ecx 为参数个数。
    rep movsd                             // 复制剩余参数。

    //////////////////////// 所有工作结束，弹出修改过的寄存器组，开始正式进入函数调用。
    popad                                 // 弹出保存的寄存器，当然，某些值可能已经被修改。
    popfd
    lea     esp, dword ptr [esp + 4]      // 再次虚弹，但这次只能操作 esp 。
...
fakecall_findrealesp:
    ...
    add     byte ptr [eax], al            // 用于标记函数结尾 00 。
    }
  }
#pragma warning(pop)
...
#pragma warning(push)
#pragma warning(disable:4100) // warning C4100: 未引用的形参。
// 补充说明：虽然 x64 下只有 fastcall 调用约定，但存在 Varargs 、非原型函数等，所以还是可能存在除 4 个默认寄存器外的其它寄存器传参，所以设计这个 fakecall 与 x86 兼容。
__declspec(naked) uint64 fakecall(const caller* nowthis, void* argv)
  {
  __asm
    {
    ...
    push    rax
    push    rbx
    mov     rbx, rcx                      // rbx 为 this 。
    push    rsp
    push    rbp
    mov     rbp, rdx                      // rbp 为 argv 。
    push    rsi
    push    rdi
    push    r10
    push    r11
    ...
    shl     rdx, 3
    sub     rsp, rdx                      // 扩展局部堆栈。
    mov     rdi, rsp                      // rdi 保存局部堆栈指针。

    rep movsq                             // 复制原始寄存器保护。

    mov     rsi, rbp                      // rsi 指向参数。

    //////////////////////// 开始修改可能存在的对象指针寄存器。
    mov     rbp, [rbx].caller_this        // rbp 为对象指针，注意后继可能用于成员函数的调用。
    test    rbp, rbp
    jz      fakecall_copyregs
    movzx   rdx, [rbx].caller_thisreg     // rdx 为对象指针寄存器指示。
    mov     cl, 16                        // 16 个寄存器　开始遍历。不清零。
fakecall_copythisregs:
    dec     rcx
    js      fakecall_copyregs             // 16 轮遍历。
    shr     rdx, 1                        // 检测对象指针寄存器指示。
    jnc     fakecall_copythisregs
    mov     qword ptr [rsp + rcx * 8], rbp

    //////////////////////// 开始修改可能存在的 r16 寄存器传参。注意之前 rsi 已经指向参数。
fakecall_copyregs:
    movzx   rdx, [rbx].caller_regs        // rdx 为寄存器传参指示。
    mov     cl, 16                        // 注意没有清零，类初始化有检测对象，保证此时 rcx >= 0 。
...
fakecall_regs_ok:
    ...
    shr     rdx, 16
    jnz     fakecall_func                 // 判定是函数地址，或者是虚成员函数偏移。
    mov     rdx, qword ptr [rdx + rbp]    // 读取虚函数表，注意不用 mov rdx, [rbp] 是为了不产生 00 编码，注意 [rbp + rdx] 也不行。
    mov     rax, qword ptr [rax + rdx]    // 读取虚成员函数。
fakecall_func:
    mov     [rsp + 8 * 9], rax            // 压入函数地址，修改是 rsp 位。 ->r15~r10 rdi rsi rbp
      
    //////////////////////// 开始复制剩余参数（除去寄存器参数）。注意在前面 rsi 已经跳过可能的寄存器参数了。而 rdi 已经很早就准备好了。
...
fakecall_findrealesp:
    ...
    add     byte ptr [rax], al            // 用于标记函数结尾 00 。
    }
  }
#pragma warning(pop)
...
(uint64(*)(const caller *, void *))&fakecall;  // 指向伪 call ，用于伪 call 代码的转移。

uint64 caller::operator()(...) const
  {
  void** ap = (void**)_AddressOfReturnAddress();
  ++ap;
#ifndef _WIN64            // x86 虽然可能默认 ecx 传递 this ，但也可能存在 this 入栈的情况。
  if(*ap == this) ++ap;
#else                     // x64 下， this 一律入栈。
  ++ap;
#endif
  return g_lp_fakecall(this, (void*)ap);
  }

static const intptr_t gk_default_argc = 0x8; // 默认操作参数个数为 0x8 个。
...
bool caller::init(void* const         func_vtno,
  ...
  {
  if(lp_this && (this_regs==arg_nul))  return false;// 如存在 this ，却没寄存器指示，出错。
  ...
  // MS 编译器好像存在有诡异的堆栈对齐假定，所以需要使其 64 位对齐。
  if((count % 8) == 0) ++count;
#endif
