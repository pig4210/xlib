...
done = xblk::NoIn == blk.check((void*)0x1111);
SHOW_TEST_RESULT;

SHOW_TEST_HEAD(xblk WholeIn);
done = xblk::WholeIn == blk.check((void*)0x1005);
SHOW_TEST_RESULT;

SHOW_TEST_HEAD(xblk HeadIn);
done = xblk::HeadIn == blk.check((void*)0x10FF, 3);
SHOW_TEST_RESULT;

SHOW_TEST_HEAD(xblk TailIn);
done = xblk::TailIn == blk.check((void*)0x0FFF, 3);
SHOW_TEST_RESULT;

SHOW_TEST_HEAD(xblk SubIn);
done = xblk::SubIn == blk.check((void*)0x0FFF, 0x1101);
SHOW_TEST_RESULT;
