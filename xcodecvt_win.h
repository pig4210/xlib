...
inline std::wstring as2ws(const std::string& as, size_t* const lpread = nullptr)
  ...
  for(rd = as.size(); rd != 0; --rd)
    {
    const auto write = MultiByteToWideChar(LOCALE_AS_WS, dwFlags, as.data(), rd, ws.data(), ws.size());
    if(write > 0)
      {
      read = rd;
      ws.resize(write);
...
inline std::string ws2as(const std::wstring& ws, size_t* const lpread = nullptr)
  ...
  for(rd = ws.size(); rd != 0; --rd)
    ...
    const auto write = WideCharToMultiByte(LOCALE_AS_WS, 0, ws.data(), rd, as.data(), as.size(), nullptr, &UsedDefultChar);
    if(write > 0)
      {
...
inline std::wstring u82ws(const std::u8string& u8, size_t* const lpread = nullptr)
  ...
  for(rd = u8.size(); rd != 0; --rd)
    {
    const auto write = MultiByteToWideChar(LOCALE_WS_U8, dwFlags, (const char*)u8.data(), rd, ws.data(), ws.size());
    if(write > 0)
      {
      read = rd;
      ws.resize(write);
...
inline std::u8string ws2u8(const std::wstring& ws, size_t* const lpread = nullptr)
  ...
  for(rd = ws.size(); rd != 0; --rd)
    {
    BOOL UsedDefultChar = FALSE;
    const auto write = WideCharToMultiByte(LOCALE_WS_U8, 0, ws.data(), rd, (char*)u8.data(), u8.size(), nullptr, &UsedDefultChar);
    if(write > 0)
      {
      if(nullptr != lpread && UsedDefultChar == TRUE) continue;
      read = rd;
