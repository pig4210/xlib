...
inline bool IsHookOK(const HOOK_ERROR_ENUM e)
  {
  return XHE_OK >= e;
  }
...
bool operator==(const ShellCodeAllocator<T>&, const ShellCodeAllocator<U>&) { return true; }
template <class T, class U>
bool operator!=(const ShellCodeAllocator<T>&, const ShellCodeAllocator<U>&) { return false; }
...
class Opcodes : public std::basic_string<char, std::char_traits<char>, Allocator>
  {
  public:
...
class xHook
  ...
  protected:
    ...
    /// UEF 回调。
    inline static LONG __cdecl UEFHandling(_EXCEPTION_POINTERS* ExceptionInfo, const xHook* o)
      {
  ...
  public:
    ...
    */
    ...
    if(routinefirst)
      ...
      switch(_hooksize)
        {
        case 1:  case 2: case 3: case 4: mkuef(); break;
        case 5:
          {
          const size_t addrdisp = CalcOffset((const char*)_hookmem + 1, _lpshellcode);
    ...
    */
    xHook(void*           hookmem,
      ...
      {
      //////////////////////////////////////////////////////////////// 基本检查 及 初始化。
      const size_t hooksize = calltable_offset ? sizeof(hookmem): sizeof(AddrDisp);
      if(!check_hookmem(hookmem, hooksize)) return;
      if(!check_routine(routine)) return;

      _ip = calltable_offset
        ? *(void**)hookmem
