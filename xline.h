/**
  ...
  - Ring0 不使用异常机制。
  - xline 类似 struct 库，但由于 C 无法返回多值，故采用此形式。
...
class xline : public std::basic_string<uint8>
  ...
  public:
    ...
    */
    ...
    /**
      先输入数据长度（输入格式由 net 决定），再输入数据内容。\n
      注意：当操作自身时，相当于 mkhead() 。
    ...
    */
    ...
    /// 扩展功能，使得 xline 可以执行一个函数。
    _Myt& operator<<(_Myt& (*pfn)(_Myt&))
      {
      return pfn(*this);
      }
    /// 扩展功能，使得不借助两个 xline 就能完成头部添加 。
    _Myt& mkhead()
      {
...
/// line 数据头为 word ，不包含自身，小端序，不处理结尾 0 。
typedef xline<uint16, false, false, false>   line;
/// netline 数据头为 word ,不包含自身，大端顺序，处理结尾 0 。
